  ...
  patterns.forEach(function(pattern) {
    ...
    for (var i = 0; i < pattern.length; i++) {
      ...
      if (!(c in trie[cur].children)) {
        ...
        steps.push({
          type: 'add-node', trie: acCloneTrie(trie), from: cur, to: newId, char: c, pattern: pattern,
          msg: 'Pattern "' + pattern + '": add node #' + newId + ' via "' + c + '" from #' + cur + '.'
        });
      } else {
        steps.push({
          type: 'reuse-node', trie: acCloneTrie(trie), from: cur, to: trie[cur].children[c], char: c, pattern: pattern,
          msg: 'Pattern "' + pattern + '": node for "' + c + '" already exists (#' + trie[cur].children[c] + ') — reuse.'
        });
      }
      cur = trie[cur].children[c];
    }
    trie[cur].output.push(pattern);
    steps.push({ type: 'mark-output', trie: acCloneTrie(trie), nodeId: cur, pattern: pattern, msg: 'Pattern "' + pattern + '" complete — mark node #' + cur + ' as an output (accepting) node.' });
  });

  steps.push({ type: 'done', trie: acCloneTrie(trie), msg: 'Trie complete: ' + trie.length + ' nodes for ' + patterns.length + ' pattern(s).' });
  return steps;
}

function acCloneTrie(trie) {
  return trie.map(function(n) {
    return { children: Object.assign({}, n.children), fail: n.fail, output: n.output.slice(), depth: n.depth };
  });
}
...
function acGenFailSteps(trie) {
  ...
  Object.keys(trie[0].children).forEach(function(c) {
    ...
    trie[childId].fail = 0;
    queue.push(childId);
    steps.push({ type: 'set-fail', trie: acCloneTrie(trie), nodeId: childId, failId: 0, msg: 'Depth-1 node #' + childId + ' ("' + c + '"): fail link → root (#0) by definition.' });
  });
  ...
  while (queue.length) {
    ...
    Object.keys(u.children).forEach(function(c) {
      ...
      steps.push({
        type: 'set-fail', trie: acCloneTrie(trie), nodeId: vId, failId: f,
        msg: 'Node #' + vId + ' (via "' + c + '" from #' + uId + '): fail link → #' + f + (trie[f].output.length ? ' (inherits output: ' + trie[f].output.join(', ') + ')' : '') + '.'
      });
    });
  }

  steps.push({ type: 'done', trie: acCloneTrie(trie), msg: 'All failure links computed via BFS. Automaton ready to search.' });
  return steps;
}
...
function acGenSearchSteps(trie, text) {
  ...
  for (var i = 0; i < text.length; i++) {
    ...
    while (cur !== 0 && !(c in trie[cur].children)) { cur = trie[cur].fail; comparisons++; }
    comparisons++;
    if (c in trie[cur].children) cur = trie[cur].children[c];
    else cur = 0;

    var matches = trie[cur].output.slice();

    steps.push({
      type: 'char', charIdx: i, char: c, fromNode: startCur, toNode: cur, matches: matches,
      msg: 'Position ' + i + ' ("' + c + '"): state #' + startCur + ' → #' + cur + (matches.length ? '. MATCH: ' + matches.join(', ') + ' ending here!' : '.'),
    });
  }

  steps.push({ type: 'done', comparisons: comparisons, msg: 'Search complete. ' + comparisons + ' character comparisons for text length ' + text.length + '.' });
  return steps;
}
...
function acRenderTrieSvg(svgId, trie, opts) {
  ...
  [['acGotoArrow','rgba(148,163,184,0.6)'], ['acFailArrow','rgba(168,85,247,0.5)']].forEach(function(pair) {
    ...
    path.setAttribute('d', 'M0,0 L8,4 L0,8 z'); path.setAttribute('fill', pair[1]);
  ...
  if (opts.showFail !== false) {
    trie.forEach(function(node, id) {
      ...
      var from = positions[id]; var to = positions[node.fail];
      var mx = (from.x + to.x) / 2; var my = Math.max(from.y, to.y) + 26;
      var path = document.createElementNS(AC_NS, 'path');
      path.setAttribute('d', 'M ' + from.x + ' ' + (from.y+14) + ' Q ' + mx + ' ' + my + ' ' + to.x + ' ' + (to.y+14));
      path.setAttribute('fill', 'none'); path.setAttribute('stroke', 'rgba(168,85,247,0.4)');
      path.setAttribute('stroke-width', '1.3'); path.setAttribute('stroke-dasharray', '4 3');
  ...
  trie.forEach(function(node, id) {
    Object.keys(node.children).forEach(function(c) {
      ...
      line.setAttribute('x1', from.x + 14); line.setAttribute('y1', from.y);
      line.setAttribute('x2', toP.x - 14);  line.setAttribute('y2', toP.y);
      ...
      label.setAttribute('x', (from.x + toP.x) / 2); label.setAttribute('y', (from.y + toP.y) / 2 - 6);
      label.setAttribute('text-anchor', 'middle'); label.setAttribute('fill', isHighlighted ? '#22c55e' : 'rgba(203,213,225,0.85)');
      label.textContent = c;
  ...
  trie.forEach(function(node, id) {
    ...
    if (isActive)      { fillColor = 'rgba(34,197,94,0.35)'; strokeColor = '#22c55e'; }
    else if (isOutput) { fillColor = 'rgba(245,158,11,0.25)'; strokeColor = '#f59e0b'; }
    ...
    if (isOutput) {
      ...
      outLabel.setAttribute('x', pos.x); outLabel.setAttribute('y', pos.y + 24);
      outLabel.setAttribute('text-anchor', 'middle'); outLabel.setAttribute('font-size', '7');
...
function acSetStatus(elId, msg, cls) {
  ...
  if (!el) return;
  el.textContent = msg; el.className = 'ac-status ' + (cls || '');
}
...
function acBuildTrie() {
  ...
  if (patterns.length === 0) { acSetStatus('acTrieStatus', 'Enter at least one pattern.', ''); return; }
  ...
  if (totalEl) totalEl.textContent = patterns.reduce(function(a,p){return a+p.length;}, 0);
...
function acAutoPlayTrie() {
  ...
  else { var b = document.getElementById('acTrieStepBtn'); if (b) b.disabled = true; }
}

function acStepTrie() {
  if (acState.trieSteps.length === 0) { acSetStatus('acTrieStatus', 'Click Build Trie first.', ''); return; }
  if (acState.trieIdx >= acState.trieSteps.length) return;
  acApplyTrieStep(acState.trieSteps[acState.trieIdx]);
  acState.trieIdx++;
  if (acState.trieIdx >= acState.trieSteps.length) { var b = document.getElementById('acTrieStepBtn'); if (b) b.disabled = true; }
}
...
function acApplyTrieStep(step) {
  ...
  acRenderTrieSvg('acTrieSvg', step.trie, { showFail: false, activeIds: activeIds, highlightEdge: step.from !== undefined && step.char ? { from: step.from, char: step.char } : null });
  acSetStatus('acTrieStatus', step.msg, step.type === 'done' ? 'done' : '');

  if (['add-node','mark-output'].indexOf(step.type) !== -1) acAddLog('acTrieLog', step.msg, step.type === 'mark-output' ? 'match' : '');
...
function acBuildFail() {
  if (acState.patterns.length === 0) { acSetStatus('acFailStatus', 'Build the trie in phase 1 first.', ''); return; }
...
function acAutoPlayFail() {
  ...
  else { var b = document.getElementById('acFailStepBtn'); if (b) b.disabled = true; }
}

function acStepFail() {
  if (acState.failSteps.length === 0) { acSetStatus('acFailStatus', 'Click Compute Failure Links first.', ''); return; }
  if (acState.failIdx >= acState.failSteps.length) return;
  acApplyFailStep(acState.failSteps[acState.failIdx]);
  acState.failIdx++;
  if (acState.failIdx >= acState.failSteps.length) { var b = document.getElementById('acFailStepBtn'); if (b) b.disabled = true; }
}

function acApplyFailStep(step) {
  var activeIds = step.nodeId !== undefined ? [step.nodeId, step.failId] : [];
...
function acRunSearch() {
  if (!acState.trieWithFail) { acSetStatus('acSearchStatus', 'Compute failure links in phase 2 first.', ''); return; }

  var input = document.getElementById('acSearchInput');
  var text  = input ? input.value : '';
  if (!text) { acSetStatus('acSearchStatus', 'Enter search text.', ''); return; }
  ...
  if (traceEl) traceEl.innerHTML = text.split('').map(function(c, i) {
...
function acAutoPlaySearch() {
  ...
  else { var b = document.getElementById('acSearchStepBtn'); if (b) b.disabled = true; }
}

function acStepSearch() {
  if (acState.searchSteps.length === 0) { acSetStatus('acSearchStatus', 'Click Search first.', ''); return; }
  if (acState.searchIdx >= acState.searchSteps.length) return;
  acApplySearchStep(acState.searchSteps[acState.searchIdx]);
  acState.searchIdx++;
  if (acState.searchIdx >= acState.searchSteps.length) { var b = document.getElementById('acSearchStepBtn'); if (b) b.disabled = true; }
}

function acApplySearchStep(step) {
  if (step.type === 'done') {
    acSetStatus('acSearchStatus', step.msg, 'done');
    var acEl = document.getElementById('acCompAc');
    if (acEl) acEl.textContent = step.comparisons;
    return;
  }

  document.querySelectorAll('.ac-trace-char').forEach(function(el) { el.classList.remove('current'); });
  var curEl = document.getElementById('acTraceChar' + step.charIdx);
  if (curEl) curEl.classList.add('current');

  acRenderTrieSvg('acSearchSvg', acState.trieWithFail, { showFail: true, activeIds: [step.toNode] });
  ...
  if (step.matches.length) {
    step.matches.forEach(function(pattern) {
      ...
      acAddLog('acMatchLog', '"' + pattern + '" found ending at position ' + endIdx + ' (chars ' + startIdx + '-' + endIdx + ')', 'match');
    });
  }
}
...
function acNewPuzzle() {
  ...
  for (var i = 0; i < chosen.text.length; i++) {
    ...
    matches.forEach(function(pattern) {
      var end = i; var start = end - pattern.length + 1;
  ...
  if (mysteryEl) {
    mysteryEl.innerHTML = chosen.text.split('').map(function(ch, i) {
      return '<span class="ac-mystery-char' + (matchedPositions[i] ? ' matched' : '') + '">' + ch + '</span>';
    }).join('');
  }
...
function acCheckGuess() {
  ...
  var guesses = acParsePatterns(input ? input.value : '').map(function(s) { return s.toLowerCase(); });
  var correct = acState.puzzlePatterns.slice().sort();
  var guessSorted = guesses.slice().sort();

  var isCorrect = correct.length === guessSorted.length && correct.every(function(p, i) { return p === guessSorted[i]; });
  ...
  if (resultEl) {
    ...
    resultEl.textContent = isCorrect
      ...
      : '❌ Not quite. You guessed: ' + guesses.join(', ') + '. Try again or click New Puzzle to see the answer.';
  }
}
...
function acInit() {
  document.querySelectorAll('.ac-phase-btn').forEach(function(btn) {
    btn.addEventListener('click', function() {
      document.querySelectorAll('.ac-phase-btn').forEach(function(b) { b.classList.remove('active'); });
      btn.classList.add('active');
      var phase = btn.getAttribute('data-phase');
      document.querySelectorAll('.ac-phase-panel').forEach(function(p) { p.classList.remove('active'); });
  ...
  if (patternsInput) patternsInput.addEventListener('keydown', function(e) { if (e.key === 'Enter') acBuildTrie(); });

  var searchInput = document.getElementById('acSearchInput');
  if (searchInput) searchInput.addEventListener('keydown', function(e) { if (e.key === 'Enter') acRunSearch(); });
